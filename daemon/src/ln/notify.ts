 ...
 * 전이 → 누구에게 무슨 알림 (웹 푸시)
 *
 * 표는 프론트 어드민의 `notify-triggers`를 옮긴 것이다. 프론트 시절엔 어드민을 새로 열 때마다 옛
 * 이벤트가 다시 와서 같은 알림이 또 울렸다(`notified-events`로 막던 것). 데몬에서는 전이가 DB 변경 한
 * 번에 한 번만 일어나므로 그 문제가 구조적으로 없다 — `notices`는 **같은 버전에서 두 번 부르는 실수**만
 * 막는 안전망이다. 키에 버전이 들어가므로, 클레임이 풀렸다 다시 승인되면 "결제하세요"는 다시 간다.
 *
