...
## 2026-09-18 — 후원자 인보이스를 에스크로 이후로 옮김

클레임 시 bolt11을 받고 유동성 프로빙에 성공해야 `verified`로 넘어가던 구조를
바꿨다. 설계 근거와 전체 공격 분석은 [docs/DESIGN-LATE-INVOICE.md](docs/DESIGN-LATE-INVOICE.md).

핵심만:

| 바뀐 것 | 이유 |
|---|---|
| `escrowed → invoiced` 신규 상태 | 인보이스 유무를 FSM이 구조적으로 강제. 핸들러 `if`에 맡기면 새 경로에서 빠뜨린다 |
| `escrowed → paid` 제거 | 지급 대상 없이 settle하면 BTC가 어드민에 묶인다 (I-010) |
| 계좌 발행이 `invoiced`부터 | 후원자가 받을 준비가 된 뒤에만 계좌가 나간다 (I-009) |
| 금액 ±5% → 정확 일치 | 금액 출처가 후원자 → 어드민으로 바뀌어 근사할 이유가 없어졌다 (I-011) |

`escrowed`/`invoiced`에서 취소를 열지 **않는다.** 계좌가 안 나갔으니 안전해
보이지만, 고객이 코드를 고쳐 계좌를 미리 뿌리고 후원자도 고친 코드로 송금하면
T-003이 그대로 부활한다. CLTV 타임아웃이 이미 교착을 해소하므로 열 이유가 없다.

## 2026-09-19 — 어드민 강제 종결(`admin_closed`) 추가

에스크로가 잡힌 채 방치된 거래를 어드민이 끊을 수 있게 했다. 그대로 두면 hold
invoice가 CLTV 타임아웃까지 유동성을 붙들고, 같은 채널의 다른 결제까지 막는다.

**`escrowed → cancelled`를 열지 않았다.** 그 경로는 T-003(선취적 취소) 그대로다.
대신 어드민만 갈 수 있는 종결 상태를 따로 만들었다 — 전이 맵에 예외를 내는 것보다
안전하다.

`invoiced`에서의 종결은 계좌가 이미 나간 뒤라 후원자가 송금했을 수 있다.
**코드로 막지 않고 경고만 한다** — 송금 완료 버튼을 안 누른 건 후원자의 불성실이고,
그걸 코드가 배려하면 정작 필요한 정리를 못 한다. 대신 주문 상세에 세 자금(에스크로·
고객 보증금·후원자 보증금)의 현 상태를 띄워 어드민이 보고 판단하게 했다.

이미 settle된 거래는 자동 종결하지 않는다(`ALREADY_SETTLED`). 조용히 닫으면
고객 돈이 증발한 것처럼 되고, 환불은 별도 결제가 필요하다.
...
### T-008. 제3자의 가짜 계좌 정보 (account-info 사칭)

| 항목 | 내용 |
|------|------|
| **공격자** | 제3자 (아무 nostr 키) |
| **시나리오** | 후원자 pubkey는 오더 태그에 공개돼 있다. 공격자가 자기 계좌를 NIP-44로 암호화해 `account-info`로 후원자에게 보낸다. 후원자 앱은 **보낸 사람 키로 복호화만 되면** 받아 화면에 띄웠다. |
| **영향** | 후원자가 **공격자 계좌로 원화를 송금**한다. 고객은 입금을 못 받았으니 릴리스하지 않고, 후원자는 원화를 잃는다. |
| **방어** | ✅ **2026-09-24 수정** — 발신자가 **그 오더의 고객**(`order.customerPubkey`)일 때만 받는다. 오더보다 먼저 도착한 계좌는 메모리에 잡아뒀다가 오더가 오면 판정한다(두 구독이 따로 돌아 순서가 뒤집힐 수 있다 — 버리면 진짜 계좌를 잃는다). |
| **발견 경위** | 온체인 트랙 구현 감사(PLAN-ONCHAIN-TRACK §14 리뷰 #8)에서 같은 모양으로 먼저 발견 — 온체인은 스토어가 "먼저 온 것 유지"라 **먼저 쏜 가짜가 진짜를 밀어냈고**, 라이트닝은 "뒤에 온 것으로 덮기"라 **나중에 쏜 가짜가 진짜를 덮었다.** 방향만 다르고 결과는 같다. |
| **관련 코드** | `customer/src/sponsor/nostr/service.ts` handleAccountInfo · 회귀 테스트 `customer/src/tests/sponsor-account-info.test.ts` |

> 온체인 트랙의 위협(공격 A~Z)은 [PLAN-ONCHAIN-TRACK.md](docs/PLAN-ONCHAIN-TRACK.md) §7이
> 진실이다. 거기서 라이트닝에도 해당하는 것만 여기 옮긴다.
