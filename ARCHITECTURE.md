...
"페어바이"는 **비트코인으로 상품을 결제하고 싶은 사람**(Customer)과,
...
nginx가 한 가지 일을 더 한다. 푸시 발송은 원래 애플리케이션 서버가 하는 일인데
이 앱은 백엔드가 없어 어드민 브라우저 탭이 그 역할을 한다. 그러면 발송 POST도
브라우저가 쏘는 게 되어 CORS에 걸리고, 브라우저 오리진을 허용하는 푸시 서비스는
모질라뿐이다(2026-09-14 실측: FCM preflight 400 / Apple OPTIONS 405, 둘 다 ACAO 없음).
파이어폭스 유저 말고는 아무도 못 받는다는 뜻이다.

```
어드민 탭 ──→ admin.도메인/push/<푸시호스트>/<경로> ──→ 푸시 서비스
              (같은 오리진 = CORS 없음)                (서버끼리 = CORS 없음)
```

중계가 하는 일은 오리진을 바꿔주는 것뿐이다. 페이로드는 구독자 공개키로 이미
암호화돼 있어(RFC 8291) nginx도 푸시 서비스도 못 읽는다. 설정은
[`deploy/nginx-push-proxy.conf`](deploy/nginx-push-proxy.conf).

주의할 점 셋:

1. **화이트리스트가 없으면 오픈 프록시**가 된다.
2. **응답 상태코드를 그대로 돌려줘야** 404/410으로 죽은 구독을 정리할 수 있다.
3. **브라우저 흔적 헤더를 지워야 한다.** nginx는 클라이언트 헤더를 전부 넘기는데,
   여기서 클라이언트는 브라우저라 `Sec-Fetch-*`가 구글까지 간다. 구글은 그걸 보고
   403 + HTML 차단 페이지를 준다(FCM API의 오류가 아니라 엣지 차단). 실측으로
   `Sec-Fetch-*`만이 원인이었고 Origin·Referer·User-Agent는 무해했지만,
   서버 대 서버 호출에 의미 없는 헤더는 같이 지운다.
