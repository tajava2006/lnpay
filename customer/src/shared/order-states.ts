 ...
 * 없으면 raw 필드로 "주문 감지"(미발행) vs "요청 대기"(발행됨)를 구분한다.
 */
...
/** 주문 감지됨 (미발행) */
const DETECTED_META: DisplayMeta = {
  label: '주문 감지',
  bgColor: '#E0E7FF',
  textColor: '#3730A3',
  isFinal: false,
 ...
 */
export function getDisplayMeta(order: TrackedOrder): DisplayMeta {
  if (order.adminState) {
    return ADMIN_STATE_META[order.adminState];
  }
  return order.raw ? PENDING_META : DETECTED_META;
}

/**
 * 최종 상태인지 확인
 */
export function isFinal(order: TrackedOrder): boolean {
 ...
 * 주문 삭제 가능 여부 확인
 */
export function isDeletable(order: TrackedOrder): boolean {
