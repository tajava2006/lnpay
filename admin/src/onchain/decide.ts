...
export type OnchainAction =
  ...
  | { kind: 'rebroadcast' }
  /** 브로드캐스트 직전 기록(outbox)이 남아 있는데 `settling`이 아니다 — 발행부터 다시 */
  | { kind: 'flush-outbox' };
...
export function decideOnchainAction(
  ...
  // 브로드캐스트 직전에 멈춘 게 있으면 그것부터 마무리한다 — 체인에 뭔가 떴을 수 있다.
  if (ctx.outboxPending && order.state !== 'settling') return { kind: 'flush-outbox' };
