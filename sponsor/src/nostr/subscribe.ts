...
export interface RequestSubscriptionCallbacks {
  /** kind 1111 request 수신 (account-info 등) */
...
/**
 * Sponsor에게 향하는 kind 1111 request를 실시간 구독한다.
 * #p=[myPubkey] 필터로 자기에게 오는 이벤트만 수신.
 *
 * 반환: cleanup 함수
 */
export function subscribeRequests(
  relays: string[],
  myPubkey: string,
  callbacks: RequestSubscriptionCallbacks,
): () => void {
  const pool = new SimplePool();
  ...
  const sub = pool.subscribeMany(
    ...
    {
      onevent: callbacks.onRequest,
      oneose: callbacks.onEose,
    },
  );

  console.log('[Nostr] Subscribed to requests for', myPubkey.slice(0, 8), 'on', relays.length, 'relays');

  return () => {
    sub.close();
    pool.destroy();
    console.log('[Nostr] Request subscription closed');
  };
}
