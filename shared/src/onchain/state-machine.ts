 ...
 * 온체인은 주소가 세 키에 커밋하므로 **후원자를 알기 전에는 주소 자체가 없다**.
 *
...
export const ONCHAIN_STATES = {
   ...
   *
   * 상태로 둔 이유: 사이드 스토어였을 때는 상태가 `funded`/`presigned`에
 ...
 * 결정도 내리지 않는다**(T-104 — 그래서 `funded`는 N컨펌 필수다). 아무 결정도
 ...
 *   상태가 아니라 **증거**다. 상태로 받으면 원화 마감 시계가 멈추고
 *   그 순간 무한 옵션이 열린다.
...
export const SETTLEMENT_KINDS = {
   ...
   *
   * **잠정 사유다.** 누구 과실인지 사람이 봐야 해서 보증금을 양쪽 다 붙잡아
 ...
 * - `hold`    — **사람이 판정할 때까지 손대지 않는다** (계좌 이의)
 *
 * ⚠️ 보증금은 **결정 시점**에 처리한다 — 종결 tx 컨펌 때가 아니다.
...
export interface OutcomeRule {
   ...
   * 어드민이 **판정 노동**을 한 종결인가.
   *
   * 몰수금의 쓰임을 가른다: 분쟁이면 **전액 중재료**, 타임아웃이면
...
/**
 * **사유가 곧 보증금 처리다**(docs/ONCHAIN-TRACK.md §4).
...
/** 몰수금의 쓰임. 분쟁이면 중재료가 우선이고 피해자 충당은 안 한다. */
 ...
 * 계속 떠 있었다**(2026-09-23).
 *
 * **진실은 FSM이다.** 어드민 핸들러도 같은 함수로 막는다 — 화면만
