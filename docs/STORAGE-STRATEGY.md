...
> 상태: **아카이브 — §6의 일부는 폐기됨** | 작성일: 2026-02-18

> ## ⚠️ 폐기된 결정 (2026-09-12)
>
> **§6의 "Customer/Sponsor는 IndexedDB 불필요 — 거래 기록이 필요하면 릴레이에서
> Admin의 이벤트를 재조회하면 된다"는 더 이상 유효하지 않다.**
>
> 같은 문서 §"만료 태그 통일"이 오더와 관련된 **모든** 이벤트에 만료 시각을
> 부여하기로 정했다. kind 30402에도 `expiration` 태그가 붙는다. 만료가 지나면
> 릴레이가 지우므로 **재조회할 대상이 남지 않는다.** 두 결정이 서로를 무력화한다.
>
> 실제로 Sponsor는 이 구멍 때문에 먼저 IDB를 도입했고(`1225e08`), Customer만
> 남아 있다가 앱 통합 시점에 같이 붙였다. 현행은 이렇다:
>
> - **localStorage** — 유계 데이터. 오더북 + 내 진행중 주문. 만료 시 삭제(60초 주기)
> - **IndexedDB** — 무계 데이터. 내가 관여한 주문 아카이브 + requests + 분쟁 채팅
>
> 역할(고객/후원자)은 별도 칼럼 없이 `customerPubkey`/`sponsorPubkey`와 내 pubkey를
> 비교해 유도한다. 한 주문에서 둘 다 참일 수 있는 경로는 자기 클레임뿐인데
> Admin FSM이 막으므로 판정이 항상 하나로 떨어진다.
>
> 아래 본문은 당시 설계 기록으로 남긴다. 현행 구조는 [ARCHITECTURE.md](../ARCHITECTURE.md)가 진실.
