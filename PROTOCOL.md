...
NIP-17 안내는 2026-09-17부터 화면에서 감췄다. Web Push가 크롬·브레이브·파이어폭스·
안드로이드까지 커버하게 되면서 "앱 하나 더 설치"를 권할 이유가 없어졌다.
**발송은 계속 한다** — 이미 키를 클라이언트에 넣어둔 사람은 그대로 받는다.
다시 켜려면 `customer/src/components/NotifySetup.tsx`의 `SHOW_NOSTR_FALLBACK`.
