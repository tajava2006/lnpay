 ...
 * 마감 시계
 *
 * 값 자체보다 **부등식**이 중요하다 — 라이트닝에서 "에스크로가 2시간 남았는데 6시간짜리 인보이스를
 * 받아줬다"가 나온 자리다. 트랙을 가로지르는 관계는 데몬의 `timing-invariants.test.ts`가 본다.
 */
...
describe('지켜야 할 부등식', () => {
   ...
   * 언젠가 추월당한다 — 그래서 **유한해야** 보증금이 의미를 갖는다.
   */
  ...
  /** 타임락(8주)은 분쟁 최장 소요보다 **한참** 길어야 한다 (T-106). */
  ...
  /**
   * "노브가 하나 더 늘지 않는다" — cosign 마감과 가격 유효창은
