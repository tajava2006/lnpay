...
```
        ...
        ┌───────────────────────┬──────────────────────┬─────────────────────┐
        │ 3838 요청·통지         │ 38383 오더 (NIP-69)   │ 3838 명령/결과        │
        │ (유저 ↔ APP)          │ (APP → 모두)          │ 33838 상태            │
        │                       │                      │ 1059 DM (APP → 운영자) │
...
- **수신**(`nostr/ingress.ts`): APP 앞으로 온 요청 이벤트(3838)를 전부 `inbox`에 넣는다. 구독은 5분마다 새로 연다 — 오래 사는
...
| `inbox` | 받은 요청 이벤트 원문과 처리 결과 (id UNIQUE) |
...
| 무엇 | 이벤트 | 방향 |
|---|---|---|
| 명령 | 3838 · `action=admin-command` · `p=APP` · 10분 TTL | 운영자 → 데몬 |
| 결과 | 3838 · `action=admin-result` · `p=운영자` · `e=명령` | 데몬 → 운영자 |
| 채팅 사본 | 3838 · `action=admin-chat` · `p=운영자` | 데몬 → 운영자 |
| 상태 | 33838 · `d=lnpay-admin:<태그>:state:<운영자>` | 데몬 → 운영자 (하트비트·설정·경보·epoch·창 길이) |
| 오더 상세 | 33838 · `d=lnpay-admin:<태그>:order:<트랙>:<오더>:<운영자>` | 데몬 → 운영자 (인보이스 상태·지급 오류·버전) |
| DM | 1059 (NIP-17) | 데몬 → 운영자 nostr 앱 (경보·분쟁 채팅·새 의뢰 — [PROTOCOL.md](PROTOCOL.md) §7) |
...
- 온체인 주문별 키는 그 키에서 HMAC으로 파생한다(추가 백업 없음).
- **키가 곧 백업이다.** 🔑 화면에서 키를 보여주고(유저가 보관), 다른 기기의 키를 가져오면 로컬을 지우고 새 키로
  다시 연다. 로컬 기록은 릴레이에서 되살아난다 — 앱이 `authors:[나]`를 구독해 내가 보낸 요청을 내 키로 풀고
  (NIP-44는 보낸 사람도 푼다) 고객 의뢰·보낸 계좌·환불 주소·클레임 값을 채운다(`nostr/own-requests.ts`,
  `onchain/nostr/own-requests.ts`). 이미 있는 기록은 덮지 않는다.
...
- **운영자 알림**은 NIP-17 DM으로 간다(`daemon/src/admin/notify.ts`, 데몬 릴레이로만) — 경보(사유마다 한 번), 유저의
  분쟁 채팅(오더마다 10분에 한 번), 새 의뢰. 다른 이벤트는 전부 우리 kind라 어떤 클라이언트도 울리지 않으므로 사람이
  알아야 할 일은 **이 길 하나**다([PROTOCOL.md](PROTOCOL.md) §7). 데몬이 죽으면 이것도 못 간다 — RISKS R-3.
