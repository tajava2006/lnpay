...
- **모든 이벤트에 `expiration`(NIP-40)** — 릴레이 찌꺼기 방지. 예외 넷:
  `dispute-message`(분쟁 증거 보존), 유저 신원 kind 0·10002(replaceable), 알림 gift wrap kind 1059,
  `push-subscription`(계정 단위라 주문보다 오래 산다).
