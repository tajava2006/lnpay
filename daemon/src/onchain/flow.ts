 ...
 * - **어드민 키는 시드 파생**이다(DM-005) — 키 저장소·백업 확인이 없어졌다.
 * - **outbox와 `settling`을 한 트랜잭션에 쓴다.** 프론트 시절엔 "발행 → 브로드캐스트" 순서를 지키려고
 *   outbox 마무리 경로가 따로 있었다. 이제 장부(DB)가 먼저 알고, 뿌리는 건 그 뒤의 효과다.
 * - 동시성 가드(`ifUnchangedSince`)가 없어졌다 — 쓰는 곳이 하나다(DM-001).
 *
 * ── 리뷰 #8의 규칙은 그대로다
 *
 * 1. **어드민은 언제나 마지막에 서명한다.** 고객에게는 서명 없는 PSBT가 가고, 고객 서명을 검증한 뒤 어드민이
 *    서명해 직접 뿌린다 — 고객 손에 완성 가능한 환불이 들리지 않는다.
 * 2. **결정은 상태다.** 환불이 결정되면 `refunding` — 늦은 사전서명·계좌·송금 주장은 전부 거절된다.
 * 3. **누가 보냈는지 본다.**
 */
import {
