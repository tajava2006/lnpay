...
|------|------|
| **유형** | 설계 한계 |
| **시나리오** | Customer가 Tampermonkey를 비활성화하거나 쿠팡 앱(모바일)에서 작업하면 유저스크립트의 자동 감지(입금 확인, 취소 감지)가 작동하지 않음. |
| **영향** | 자동 `payment-confirm`이나 `cancel-request`가 발행되지 않아 상태 전이가 지연됨. |
| **현재 처리** | Customer가 수동으로 입금 확인 버튼을 누르거나, 궁극적으로 hold invoice CLTV timeout이 안전장치 역할. |
